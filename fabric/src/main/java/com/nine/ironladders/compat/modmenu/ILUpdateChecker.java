...
import com.nine.ironladders.ILFabric;
...
public class ILUpdateChecker implements UpdateChecker {
    ...
    public UpdateInfo checkForUpdates() {
        ...
        var updateVersions = getUpdateVersions(SharedConstants.getCurrentVersion().getName(), updateInfo);
        VersionUpdateData update = null;
        var tempVersion = ILCommon.MOD_VERSION;
        ...
        return update == null ? UpdateInfo.NONE :
                new UpdateInfo(true, Component.literal(update.version()), update.url(), update.updateChannel());
    }

    public static JsonObject tryGetUrlInfo() {
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create(ILFabric.UPDATE_JSON_URL))
            ...
            ILCommon.LOGGER.debug("Couldn't fetch update info: {}", e.getMessage());
        }
        return null;
    }
