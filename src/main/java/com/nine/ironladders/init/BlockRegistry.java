...
public class BlockRegistry {
    ...
    public static final RegistryObject<Block> DIAMOND_LADDER = register("diamond_ladder",
            () -> new DiamondLadderBlock(BlockBehaviour.Properties.of().forceSolidOff().strength(3.0F, 6.0F).sound(SoundType.METAL).noOcclusion().mapColor(MapColor.DIAMOND).pushReaction(PushReaction.DESTROY)));
    public static final RegistryObject<Block> NETHERITE_LADDER = register("netherite_ladder",
            () -> new NetheriteLadderBlock(BlockBehaviour.Properties.of().forceSolidOff().strength(5.0F, 1200.0F).sound(SoundType.NETHERITE_BLOCK).noOcclusion().mapColor(MapColor.COLOR_BLACK).pushReaction(PushReaction.DESTROY)));
