...
public class WTHITBlockComponentProvider implements IBlockComponentProvider {
	
	@Override
	public void appendBody(ITooltip tooltip, IBlockAccessor blockAccessor, IPluginConfig config) {
		...
		if (state.hasProperty(MetalLadderBlock.HAS_SENSOR)
			...
			tooltip.addLine(Component.translatable("item.ironladders.ladder_sensor_tool.waila.block.state")
		...
		if (stack.getItem() instanceof StylerToolItem){
			...
			if (modelType != null) {
				tooltip.addLine(Component.translatable("item.ironladders.ladder_styler_tool.waila.block.style",
		...
		if (stack.getItem() instanceof MorphToolItem) {
			if (MorphToolItem.morphStateId(stack) == 0) {
				tooltip.addLine(Component.translatable("item.ironladders.ladder_morph_tool.waila.block.copy",
								ILUI.withColor(ILUI.Text.LBM, ILUI.Color.SOFT_GRAY))
						.withStyle(ChatFormatting.GRAY));
			}
			var morph = be.morphState();
			if (morph != null) {
				tooltip.addLine(Component.translatable("item.ironladders.ladder_morph_tool.waila.block.current",
		...
		if (stack.getItem() instanceof CasingToolItem) {
			...
			tooltip.addLine(Component.translatable("item.ironladders.ladder_casing_tool.waila.block.state",
							ILUI.withColor(
